...
// ---------------------------------------------------------------------------
// UT-LAY-01：黄金图 × 3 → 四 tag 检出、IoU ≥ 0.7（需自训练权重 models/layout-wechat）
// ---------------------------------------------------------------------------
...
fn ut_lay_01_golden_regions_iou() {
    ...
    /// 模型根：仓库根的 models/（权重 gitignore，按 models/README.md 备置）。
    const MODELS_ROOT: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../../models");
    let model_dir = Path::new(MODELS_ROOT).join("layout-wechat");
    if !model_dir.join("model.toml").is_file() {
        eprintln!("跳过 UT-LAY-01：models/layout-wechat 未训练（tools/training/train_layout.py）");
        return;
    }
