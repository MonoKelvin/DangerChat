//! M4 ocr 模块测试（UT-OCR-01~05；02~05 在 src/ocr/assemble.rs 单测）。
//!
//! UT-OCR-01：黄金切片 20 张 → 字段级准确率 ≥ 95%（CI 门禁）。
//! 模型三件套不在库内（models/README 记下载源）——运行期检查，缺权重时
//! skip 并提示，本地备好权重后 `cargo test` 自动全跑。
